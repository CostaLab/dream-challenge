...
LABELS_DIR = "/hpcwork/izkf/projects/dream_tfbs/local/ChIPseq/labels/"
...
challenge_list = ["test", "final"]
chrom_list = list()


def get_features_data(factor, cell_list):
    for challenge in challenge_list:
        if challenge == "train":
            chrom_list = ["chr2", "chr22", "chr9"]
            for cell in cell_list:
                for chrom in chrom_list:
                    all_fearues_fname = os.path.join(
                        TRAINING_FEATURES_DIR, "T.{}.{}.{}.tab".format(factor, cell, chrom))
                    for i in range(1, 11):
                        sub_features_fname = os.path.join(
                            TRAINING_FEATURES_DIR, "T.{}.{}.{}.{}.tab".format(factor, cell, chrom, str(i)))
                        os.system("sed 1d " + sub_features_fname + " | cat >> " + all_fearues_fname)
                        os.remove(sub_features_fname)
        elif challenge == "final":
            ...
            for cell in cell_list:
                for chrom in chrom_list:
                    all_fearues_fname = os.path.join(
                        FINAL_FEATURES_DIR, "F.{}.{}.{}.tab".format(factor, cell, chrom))
                    os.remove(all_fearues_fname)
                    for i in range(1, 11):
                        sub_features_fname = os.path.join(
                            FINAL_FEATURES_DIR, "F.{}.{}.{}.{}.tab".format(factor, cell, chrom, str(i)))
                        os.system("sed 1d " + sub_features_fname + " | cat >> " + all_fearues_fname)
                        os.remove(sub_features_fname)
...
if __name__ == '__main__':
    ...
    get_features_data(factor, cell_list)
